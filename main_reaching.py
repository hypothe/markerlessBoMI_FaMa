...
class BoMIReaching(JointMapper):
	...
	def __init__(self, win, nmap_components, *args, **kwargs):
		...
		self.lbl_tgt.grid(row=4, column=2, columnspan=2,
						pady=GUIStyle['lbl_pady'],sticky='w')


		# mouse control
		
		self.check_mouse = tk.BooleanVar()
		self.check_m1 = tk.Checkbutton(win, text="Mouse Control", variable=self.check_mouse, command=self.mouse_check, 
									activeforeground=GUIStyle['activeforeground'],
									activebackground=GUIStyle['activebackground'],
									bg=GUIStyle['bg'])
		self.check_m1.config(font=("Times", self.font_size))
		self.check_m1.grid(row=6, column=1, pady=GUIStyle['check_pady'], sticky='w')

		# keyboard

		self.check_kb = tk.BooleanVar()
		self.check_kb1 = tk.Checkbutton(win, text="External Keyboard", variable=self.check_kb,
										activeforeground=GUIStyle['activeforeground'],
										activebackground=GUIStyle['activebackground'],
										bg=GUIStyle['bg'])
		self.check_kb1["state"]="disabled"
		self.check_kb1.config(font=("Times", self.font_size))
		self.check_kb1.grid(row=6, column=2, columnspan=2,
