...
> NOTE on the docker naming: Right now the `latest` and the `nocam` tags point to the same image, but there is no guarantee for this to remain the case in the future. Always refere to the `latest` tag unless instructed otherwise.